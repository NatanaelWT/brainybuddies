								...
								<ul id="thumbs" class="portfolio">
									...
									while ($data = mysqli_fetch_array($temp)) { ?>
										<li class="item-thumbs col-lg-3 design" data-id="id-0" data-type="<?= $data['mata_pelajaran'] ?>">
											...
											<div class="caption">
												<h3><a href="course/<?= $data['nama_materi'] ?>"><?= $data['nama_materi'] ?></a><?php if (isset($_SESSION['email'])) { ?><a href="/<?= $link[1] ?>/edit/<?= $data['id_materi'] ?>"><button class="btn-success" style="border-radius: 100%;"><i class="fa fa-cog" aria-hidden="true"></i></button></a><?php } ?></h3>
